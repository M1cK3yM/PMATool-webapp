...
import UnitConversionDialog from "./UnitConversion-dialog"
import { useState } from "react"

export default function TopMenubar() {
  const router = useRouter()
  const [unitConversionOpen, setUnitConversionOpen] = useState(false)
  const username = "Administrator" // This should be replaced with actual user management
  ...
  return (
    <>
      <Menubar className="rounded-md border">
        <MenubarMenu>
          ...
          <MenubarContent>
            ...
            <MenubarItem onClick={() => setUnitConversionOpen(true)}>
              <FolderOpen className="mr-2 h-4 w-4" /> Unit Conversion
            </MenubarItem>
            <MenubarSeparator />
      ...
      <UnitConversionDialog open={unitConversionOpen} onOpenChange={setUnitConversionOpen} username={username} />
    </>
  )
}
